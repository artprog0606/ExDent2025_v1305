    ...
    public class GlobalStringVariableService
    {
        public static List<SeoToken> GetGlobalVariables()
        {
            return new List<SeoToken>()
            {
                ...
                new SeoToken("#GEO_NAME#", GetGeoName())
            };
        }
        ...
        private static string GetGeoName()
        {
            ...
            var geoNames = DomainGeoLocationService.GetList();
                
            var geoName = 
                geoNames.Find(x => x.Url.Equals(url.Host, StringComparison.OrdinalIgnoreCase)) ?? 
                geoNames.Find(x => x.Url.Equals(url.Authority, StringComparison.OrdinalIgnoreCase));
