...
namespace AdvantShop.Saas
{
    public class SaasDataService
    {
        ...
        private const int ValidationClockSkewInMinutes = 3 * 60;
        ...
        public static SaasData CurrentSaasData
        {
            get
            {
                ...
                if (!CacheManager.TryGetValue(SaasDataKey, out saasData))
                {
                    ...
                    _ = saasData.AllServicesRemainsFormattedHash;
        ...
        public static bool IsEnabledFeature(Func<SaasData, bool> funcCheckFeature) 
            => !IsSaasEnabled || funcCheckFeature(CurrentSaasData);
                    ...
                    if (dataStream != null)
                        using (var reader = new StreamReader(dataStream))
                        {
                            ...
                            if (!string.IsNullOrEmpty(responseFromServer))
                            {
                                ...
                                serverData.ValidTill = DateTime.Now.AddMinutes(ValidationClockSkewInMinutes);
        ...
        private static void DeactivateNotInTariff(SaasData saasData)
        {
            ...
            WarehouseService.DeactivateStorageMoreThan(saasData.WarehouseAmount);
            
            if (!saasData.PriceTypes)
                PriceRuleService.Deactivate();
            if (!saasData.CustomerTypes)
            {
                SettingsCustomers.IsRegistrationAsLegalEntity = false;
                SettingsCustomers.IsRegistrationAsPhysicalEntity = true;
            ...
            if (!saasData.CustomerAdditionFields)
                CustomerFieldService.DisableAllCustomerFields();
        }
    }
}
