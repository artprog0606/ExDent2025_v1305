...
namespace AdvantShop.Core.Services.Shipping.Grastin.Api
{
    public class GrastinApiService
    {
        ...
        public List<SelfpickupBoxberry> GetBoxberrySelfPickup(string filterCity)
        {
            if (string.IsNullOrEmpty(filterCity))
                return CacheManager.Get(
                    $"GrastinApi-{_apiKey}-{EnApiMethod.BoxberrySelfPickup}",
                    60,
                    () =>
                    {
            ...
            return CacheManager.Get(
                $"GrastinApi-{_apiKey}-{EnApiMethod.BoxberrySelfPickup}-{filterCity}",
                5,
                () =>
                {
                    var response =
        ...
        public List<PostcodeBoxberry> GetBoxberryPostCode(string filterCity)
        {
            if (string.IsNullOrEmpty(filterCity))
                return CacheManager.Get(
                    $"GrastinApi-{_apiKey}-{EnApiMethod.BoxberryPostCode}",
                    10,
                    () =>
                    {
            ...
            return CacheManager.Get(
                $"GrastinApi-{_apiKey}-{EnApiMethod.BoxberryPostCode}-{filterCity}",
                5,
                () =>
                {
                    var response =
        ...
        /// </summary>
        /// <returns></returns>
        public List<SelfpickupCdek> GetCdekSelfPickup()
        {
            return GetCdekSelfPickup(string.Empty);
        }
        ...
        /// <param name="filterCity">Часть строки населенного пункта для фильтрации.</param>
        /// <returns></returns>
        public List<SelfpickupCdek> GetCdekSelfPickup(string filterCity)
        {
            if (string.IsNullOrEmpty(filterCity))
                return CacheManager.Get(
                    $"GrastinApi-{_apiKey}-{EnApiMethod.CdekSelfPickup}",
                    60,
                    () =>
                    {
                        var responseCache = MakeRequest<GrastinCdekSelfPickupResponse, GrastinCdekSelfPickup>(
                            new GrastinCdekSelfPickup());

                        if (responseCache != null)
                            return responseCache.SelfpickupsCdek;

                        return null;
                    });

            return CacheManager.Get(
                $"GrastinApi-{_apiKey}-{EnApiMethod.CdekSelfPickup}-{filterCity}",
                5,
                () =>
                {
                    var response =
                        MakeRequest<GrastinCdekSelfPickupResponse, GrastinCdekSelfPickup>(new GrastinCdekSelfPickup()
                        {
                            FilterCity = filterCity
                        });

                    if (response != null)
                        return response.SelfpickupsCdek;
        ...
        /// <summary>
        /// Получение списка городов доставки СДЭК
        /// </summary>
        /// <returns></returns>
        public List<CityCdek> GetCdekCities()
        {
            return CacheManager.Get(
                $"GrastinApi-{_apiKey}-{EnApiMethod.CdekCities}",
                60,
                () =>
                {
                    var responseCache = MakeRequest<GrastinCdekCitiesResponse, GrastinCdekCities>(
                        new GrastinCdekCities());

                    if (responseCache != null)
                        return responseCache.CitiesCdek;
        ...
        public List<SelfpickupHermes> GetHermesSelfPickup(string filterCity)
        {
            if (string.IsNullOrEmpty(filterCity))
                return CacheManager.Get(
                    $"GrastinApi-{_apiKey}-{EnApiMethod.HermesSelfPickup}",
                    60,
                    () =>
                    {
            ...
            return CacheManager.Get(
                $"GrastinApi-{_apiKey}-{EnApiMethod.HermesSelfPickup}-{filterCity}",
                5,
                () =>
                {
                    var response =
        ...
        public List<SelfpickupPartner> GetPartnerSelfPickups(string filterCity)
        {
            if (string.IsNullOrEmpty(filterCity))
                return CacheManager.Get(
                    $"GrastinApi-{_apiKey}-{EnApiMethod.PartnerSelfPickup}",
                    60,
                    () =>
                    {
            ...
            return CacheManager.Get(
                $"GrastinApi-{_apiKey}-{EnApiMethod.PartnerSelfPickup}-{filterCity}",
                5,
                () =>
                {
                    var response =
        ...
        /// <summary>
        /// Добавление заказа СДЭК
        /// </summary>
        /// <returns></returns>
        public List<OrderResponse> AddCdekOrder(CdekOrderContainer order)
        {
            var response =
                MakeRequest<CdekOrderResponse, CdekOrderContainer>(order);
            if (response != null)
                return response.OrdersResponse;
        ...
        /// </summary>
        /// <returns></returns>
        public List<CostResponse> CalcShippingCost(CalcShippingCostContainer data)
        {
            var response =
                MakeRequest<CalcShippingCostResponse, CalcShippingCostContainer>(data);
            if (response != null)
