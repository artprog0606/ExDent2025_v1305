...
namespace AdvantShop.Shipping.Usps
{
    ...
    public class Usps : BaseShipping
    {
        ...
        private readonly float _totalItemsPrice;

        public Usps(ShippingMethod method, ShippingCalculationParameters calculationParameters)
            : base(method, calculationParameters)
        {
            ...
            _totalItemsPrice = _items.Sum(item => item.Price * item.Amount);
        ...
        protected override IEnumerable<BaseShippingOption> CalcOptions(CalculationVariants calculationVariants)
        {
            try
            {
                var totalWeight = _calculationParameters.TotalWeight ?? _items.Sum(x => x.Weight*x.Amount);
        ...
        private string GetXmlInternationalPackage()
        {
            ...
            sb.AppendFormat("<ValueOfContents>{0}</ValueOfContents>", _totalItemsPrice);
                ...
                do
                {
                    ...
                    if ((tr.Name == serviceTag) && (tr.NodeType == XmlNodeType.Element))
                    {
                        ...
                        if ((EnabledService.Contains(serviceCode)) && (shippingOptions.Find(s => s.NameRate == serviceCode) == null))
                        {
                            ...
                            var shippingOption = new BaseShippingOption(_method, _totalItemsPrice)
                                                     {
                                                         Rate = shippingRate,
