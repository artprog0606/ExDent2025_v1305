...
namespace AdvantShop.Core.Services.Bonuses.Model.Rules
{
    ...
    public class ChangeGradeRule : BaseRule
    {
        ...
        {
            var cgrule = CustomRuleService.Get(ERule.ChangeGrade);
            if (cgrule == null || !cgrule.Enabled) return;
            var rule = BaseRule.Get(cgrule) as ChangeGradeRule;
            if (rule == null) return;
        ...
        private static void UpdateCard(ChangeGradeRuleDto card, Grade grade, string companyname)
        {
            ...
            NotificationService.Process(card.CardId, ENotifcationType.ChangeGrade, new OnUpgradePercentTempalte
            {
