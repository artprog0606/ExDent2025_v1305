    ...
    public class OrderHistoryService
    {
        public static void NewOrder(Order order, OrderChangedBy changedBy)
        {
            Add(new OrderHistory(changedBy)
            {
                ...
                Parameter = LocalizationService.GetResource("Core.Orders.OrderHistory.OrderCreated") + order.Number
            });
        }
        ...
        public static bool ChangingPickPoint(int orderId, OrderPickPoint pickPoint, OrderChangedBy changedBy)
        {
            var oldPickPoint = OrderService.GetOrderPickPoint(orderId);

            var oldValue = oldPickPoint?.ToString();
            var newValue = pickPoint?.ToString();
            ...
            {
                OrderId = orderId,
                Parameter = LocalizationService.GetResource("Core.Orders.Order.OrderPickPoint"),
                OldValue = oldValue,
                NewValue = newValue
            });
            return true;
            ...
            if (history.Count > 0)
            {
                ...
                if (newItem != null)
                    description = string.Join(" ", 
                        new[] { newItem.Name, newItem.Size, newItem.Color }.Where(x => !string.IsNullOrEmpty(x)));
                else if (oldItem != null)
                    description = string.Join(" ", 
                        new[] { oldItem.Name, oldItem.Size, oldItem.Color }.Where(x => !string.IsNullOrEmpty(x)));
                
                var value = newItem != null ? (newItem.ProductID ?? newItem.BookingServiceId) : null;
        ...
        {
            var changedBy =
                !string.IsNullOrEmpty(statusHistory.CustomerName) && statusHistory.CustomerID == null
                    ? new OrderChangedBy(statusHistory.CustomerName) { CustomerId = statusHistory.CustomerID }
                    : null;
        ...
        public static bool ChangingAdminOrderFiles(int orderId, List<AdminOrderAttachment> oldAttachments, List<AdminOrderAttachment> newAttachments, OrderChangedBy changedBy)
        {
            var noChanges = oldAttachments.Count == newAttachments.Count;
            if (noChanges)
                for (int i = 0; i < oldAttachments.Count; i++)
                {
                    if (oldAttachments[i].FileName != newAttachments[i].FileName
                        || oldAttachments[i].FileSize != newAttachments[i].FileSize)
                    {
                        noChanges = false;
                        break;
                    }
            ...
            {
                OrderId = orderId,
                Parameter = LocalizationService.GetResource("Core.Orders.Order.ChangingAdminOrderFiles"),
                OldValue = string.Join(", ", oldAttachments.Select(x => x.FileName)),
                NewValue = string.Join(", ", newAttachments.Select(x => x.FileName))
            });
            return true;
        }

        public static bool ChangingCustomerOrderFiles(int orderId, List<CustomerOrderAttachment> oldAttachments, List<CustomerOrderAttachment> newAttachments, OrderChangedBy changedBy)
        {
            var noChanges = oldAttachments.Count == newAttachments.Count;
            if (noChanges)
                for (int i = 0; i < oldAttachments.Count; i++)
            ...
            {
                OrderId = orderId,
                Parameter = LocalizationService.GetResource("Core.Orders.Order.ChangingCustomerOrderFiles"),
                OldValue = string.Join(", ", oldAttachments.Select(x => x.FileName)),
                NewValue = string.Join(", ", newAttachments.Select(x => x.FileName))
            });
            return true;
