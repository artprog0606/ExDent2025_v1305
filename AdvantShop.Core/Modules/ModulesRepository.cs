...
namespace AdvantShop.Core.Modules
{
    public class ModulesRepository
    {
        ...
        public static void InstallModuleToDb(Module module)
        {
            ...
            ModulesService.ClearModuleSalesChannelsCache();
        }
        ...
        public static Module GetModuleFromDb(string moduleStringId)
        {
            return SQLDataAccess.ExecuteReadOne<Module>(
                @"SELECT TOP 1 * FROM [dbo].[Modules] WHERE [ModuleStringID] = ModuleStringID",
                CommandType.Text,
                GetModuleFromReader,
        ...
        public static void SetActiveModule(string moduleStringId, bool active)
        {
            ...
            if (module != null)
            {
                ...
                {
                    CacheManager.RemoveByPattern("squishit_css");
                    CacheManager.RemoveByPattern("squishit_js");
                }
            }

            var modulesAllowOnTechDomain = AttachedModules.GetModules<IModuleAllowOnTechDomain>();
            SettingsMain.IsTechDomainPicturesAllowed = modulesAllowOnTechDomain != null && modulesAllowOnTechDomain.Count > 0;
        }
        ...
        public static bool IsPreviewShowed(string moduleStringId)
        {
            return ModuleSettingsProvider.GetSettingValue<bool>("IsPreviewShowed",moduleStringId);
        }
        
        public static void SetPreviewShowed(string moduleStringId, bool showed)
        {
            ModuleSettingsProvider.SetSettingValue("IsPreviewShowed", showed, moduleStringId);
            ModulesService.ClearModuleSalesChannelsCache();
        }
