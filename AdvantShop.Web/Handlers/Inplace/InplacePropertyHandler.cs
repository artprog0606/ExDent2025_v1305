...
namespace AdvantShop.Handlers.Inplace
{
    public class InplacePropertyHandler
    {
        public bool AddProperty(int productId, string name, string value, int propertyId = 0, int propertyValueId = 0)
        {
            ...
            if (property == null)
            {
                property = new Property()
                {
                    ...
                    UseInDetails = true,
                    UseInFilter = true,
                    Type = (int)PropertyType.Checkbox
                };

                property.PropertyId = PropertyService.AddProperty(property);
            }

            if (property.Type == (int)PropertyType.Textarea && PropertyService.IsExistPropertyInProduct(productId, property.PropertyId))
                return false;
        ...
        public object UpdateProperty(int productId, int propertyValueId, string propertyValue)
        {
            ...
            if (PropertyService.GetProductsCountByPropertyValue(propertyValueId) == 0)
            {
                oldValue.Value = propertyValue;
                PropertyService.UpdatePropertyValue(oldValue);
                newValue = oldValue;
            }
            else
            {
                newValue = PropertyService.GetPropertyValueByName(oldValue.PropertyId, propertyValue);
            }
