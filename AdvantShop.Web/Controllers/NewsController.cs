...
namespace AdvantShop.Controllers
{
    ...
    public partial class NewsController : BaseClientController
    {
        ...
        public ActionResult NewsCategory(string url, int? page)
        {
            ...
            if (model.News.Count > 0 && model.News[0]?.Picture != null)
                og.Image = model.News[0].Picture.ImageSrc();
        ...
        public ActionResult NewsItem(string url)
        {
            ...
            {
                HowENewsProducts = newsItem.ProductsDisplayMode,
                Products = new ProductViewModel(
                    NewsService.GetNewsProductModels(newsItem.NewsId, WarehouseContext.GetAvailableWarehouseIds()),
                    SettingsDesign.IsMobileTemplate)
                {
                    CountProductsInLine =
                        newsItem.ProductsDisplayMode == News.NewsItem.ENewsProductsDisplayMode.Horizontally ? 3 : 1,
            ...
            {
                metaNews.Image = model.NewsItem.Picture.ImageSrc();
            }
        ...
        {
            if ((SettingsDesign.IsMobileTemplate && !SettingsMobile.ShowNewsOnMainPage) ||
                (!SettingsDesign.IsMobileTemplate && !SettingsDesign.NewsVisibility))
        ...
        public JsonResult Subscribe(string email, bool? agreeForPromotionalNewsletter, bool? agree)
        {
            if (SettingsDesign.ShowUserAgreementForPromotionalNewsletter
                && agreeForPromotionalNewsletter != null
                && !agreeForPromotionalNewsletter.Value)
                return Json(new { status = "error", agreeForPromotionalNewsletter = "none" });
