(function (ng) {
    ...
    var UiGridCustomSelectionCtrl = function ($http, $q, $timeout, $element, toaster, $translate) {
        var ctrl = this;
        ...
        ctrl.$onInit = function () {
            ...
            var destroyEventRowChange =
                ctrl.gridApi.selection == null
                    ? function () {}
                    : ctrl.gridApi.selection.on.rowSelectionChanged(null, function (row, event) {
            ...
            var destroyEventRowChangeBatch =
                ctrl.gridApi.selection == null
                    ? function () {}
                    : ctrl.gridApi.selection.on.rowSelectionChangedBatch(null, function (rows, event) {
                          $timeout(function () {
        ...
        ctrl.addItemInStorage = function (rows) {
            let _rows = ng.isArray(rows) ? rows : rows != null ? [rows] : [],
                index;
        ...
        ctrl.getRowsFromStorage = function () {
            return ctrl.storageSelectedRows.map(function (row) {
                return row.entity;
            });
        };
        //#endregion
    ...
    UiGridCustomSelectionCtrl.$inject = ['$http', '$q', '$timeout', '$element', 'toaster', '$translate'];

    ng.module('uiGridCustomSelection', ['uiModal', 'jsTree.directive']).controller('UiGridCustomSelectionCtrl', UiGridCustomSelectionCtrl);
})(window.angular);
