...
namespace AdvantShop.Web.Admin.Controllers.Settings
{
    ...
    public partial class PaymentMethodsController : BaseAdminController
    {
        ...
        {
            var listTypes = AdvantshopConfigService.GetDropdownPayments();
            var listTypesModules = ModulesExecuter.GetDropdownPayments();
            ...
            foreach (var payment in PaymentService.GetAllPaymentMethods(false))
            {
                var typeQ =
                    listTypes.Where(p =>
                        string.Equals(p.Value, payment.PaymentKey, StringComparison.OrdinalIgnoreCase));
                ...
                if (type == null)
                    type = listTypesModules.FirstOrDefault(x => string.Equals(x.Value, payment.PaymentKey, StringComparison.OrdinalIgnoreCase));
                result.Add(new AdminPaymentMethodItemModel()
                {
                    ...
                    WarningMessage = payment.ModuleStringId.IsNotEmpty()
                        ? (module = modules.FirstOrDefault(x => string.Equals(x.StringId, payment.ModuleStringId, StringComparison.OrdinalIgnoreCase))) == null
                            ? "Модуль не установлен"
                            : module.Active is false && payment.Enabled
        ...
        {
            var list = AdvantshopConfigService.GetDropdownPayments().Select(x => 
                new InfoByPaymentTypeModel { Label = x.Text, Value = x.Value, Code = x.Code, Type = "adv-core" }).ToList();
            list.AddRange(ModulesExecuter.GetDropdownPayments().Select(x => 
                new InfoByPaymentTypeModel { Label = x.Text, Value = x.Value, Code = x.Code, Type = "exist-module" }));
            list.AddRange(
                ModulesService.GetModulePayments()
                              .Where(paymentModule =>
                                   list.All(x =>
                                       !string.Equals(x.Value,
                                           paymentModule.PaymentKey,
                                           StringComparison.OrdinalIgnoreCase)))
                              .Select(x => new InfoByPaymentTypeModel
                               {
                                   Label = x.Name,
                                   Value = x.PaymentKey,
        ...
        public JsonResult AddPaymentMethod(string name, string type, string description, string code)
        {
            ...
            Core.Modules.Interfaces.IPaymentMethod moduleOfPayment = null;

            var advPaymentType =
                    AdvantshopConfigService.GetDropdownPayments(onlyActive: false)
                                           .FirstOrDefault(p =>
                                                string.Equals(p.Value, type, StringComparison.OrdinalIgnoreCase));

            if (advPaymentType is null)
                moduleOfPayment = AttachedModules.GetModules<Core.Modules.Interfaces.IPaymentMethod>(ignoreActive: true)
                                                 .Where(module => module != null)
                                                 .Select(module => (Core.Modules.Interfaces.IPaymentMethod)Activator.CreateInstance(module))
                                                 .FirstOrDefault(module => string.Equals(module.PaymentKey, type, StringComparison.OrdinalIgnoreCase));


            if (moduleOfPayment is null
                && advPaymentType is null)
                return JsonError(string.Empty);

            method.ModuleStringId = moduleOfPayment?.ModuleStringId;
            ...
            try
            {
                ...
                if (method is Bill && 
                    (!SaasDataService.IsSaasEnabled || SaasDataService.CurrentSaasData.CustomerTypes))
                {
                    method.CustomerType = (int)CustomerType.LegalEntity;
                    var parameters = method.Parameters;
                    parameters[BillTemplate.GetCustomerDataMethod] = EGetCustomerDataMethod.FromAdditionalFields.ToString();
                    method.Parameters = parameters;
                }
                else
                    method.CustomerType = (int)CustomerType.All;
                ...
                if (method.PaymentKey == AttributeHelper.GetAttributeValue<PaymentKeyAttribute, string>(typeof(Robokassa)))
                    Track.TrackService.TrackEvent(Track.ETrackEvent.Core_Settings_RobokassaPaymentMethodCreated);

                return JsonOk(new { id = methodId });
            }
            catch (Exception ex)
            {
                Debug.Log.Error(ex);
        ...
        public ActionResult Edit([ModelBinder(typeof(ModelTypeBinder))] PaymentMethodAdminModel model)
        {
            ...
            try
            {
                ...
                method.CustomerType = (int)model.CustomerType;
                method.Parameters = model.Parameters;

                PaymentService.UpdatePaymentMethod(method);
                
                Track.TrackService.TrackEvent(Track.ETrackEvent.Core_Settings_PaymentMethodEdited, method.PaymentKey);
        ...
        public JsonResult UploadBillStamp(int methodId)
        {
            ...
            try
            {
                ...
                if (!file.FileName.Contains(".") || !FileHelpers.CheckFileExtensionByType(file.FileName, EFileType.Image))
                {
