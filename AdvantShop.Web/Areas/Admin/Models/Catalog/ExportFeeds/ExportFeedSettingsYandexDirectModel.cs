...
{
    public class ExportFeedSettingsYandexDirectModel : IValidatableObject
    {
        public ExportFeedSettingsYandexDirectModel(ExportFeedYandexDirectOptions exportFeedYandexOptions)
        {
           ...
           //GlobalDeliveryCost = exportFeedYandexOptions.GlobalDeliveryCost;
            ...
            Promos = new List<ExportFeedYandexDirectPromo>();
            ...
            ExportCollections = exportFeedYandexOptions.ExportCollections;
            ...
            try
            {
                ...
                {
                    Promos =
                        JsonConvert.DeserializeObject<List<ExportFeedYandexDirectPromo>>(exportFeedYandexOptions.Promos);

                    PromosJson = exportFeedYandexOptions.Promos;


                    PromoCodesJson = JsonConvert.SerializeObject(Promos.Where(x => x.Type == YandexDirectPromoType.PromoCode));
                    FlashDiscountsJson = JsonConvert.SerializeObject(Promos.Where(x => x.Type == YandexDirectPromoType.Flash));
                    PromoGiftsJson = JsonConvert.SerializeObject(Promos.Where(x => x.Type == YandexDirectPromoType.Gift));
                    NPlusMJson = JsonConvert.SerializeObject(Promos.Where(x => x.Type == YandexDirectPromoType.NPlusM));
                }
            }
            finally
            {
            ...
            {
                LocalDeliveryOption =
                    JsonConvert.DeserializeObject<ExportFeedYandexDirectDeliveryCostOption>(exportFeedYandexOptions.LocalDeliveryOption);
            }
            catch (Exception)
            {
            }
            ...
            GlobalDeliveryCost = new List<ExportFeedYandexDirectDeliveryCostOption>();
            try
            {
                if (!string.IsNullOrWhiteSpace(exportFeedYandexOptions.GlobalDeliveryCost))
                {
                    GlobalDeliveryCost =
                        JsonConvert.DeserializeObject<List<ExportFeedYandexDirectDeliveryCostOption>>(exportFeedYandexOptions.GlobalDeliveryCost);
        ...
        public List<ExportFeedYandexDirectPromo> Promos { get; set; }
        ...
        public ExportFeedYandexDirectDeliveryCostOption LocalDeliveryOption { get; set; }

        public List<ExportFeedYandexDirectDeliveryCostOption> GlobalDeliveryCost { get; set; }

        public string GlobalDeliveryCostJson { get; set; }

        public Dictionary<ExportFeedYandexDirectDeliveryCost, string> DeliveryCostList
        {
            get
            {
                var deliveryCostList = new Dictionary<ExportFeedYandexDirectDeliveryCost, string>();
                foreach (ExportFeedYandexDirectDeliveryCost deliveryCost in Enum.GetValues(typeof(ExportFeedYandexDirectDeliveryCost)))
                {
        ...
        public Dictionary<string, string> Currencies
        {
            get
            {
                ...
                foreach (var item in CurrencyService.GetAllCurrencies().Where(item => ExportFeedYandexDirect.AvailableCurrencies.Contains(item.Iso3)).ToList())
                {
        ...
        public Dictionary<string, string> DeliveryCostTypes =>
            new Dictionary<string, string> {
                {ExportFeedYandexDirectDeliveryCost.None.ToString(),ExportFeedYandexDirectDeliveryCost.None.Localize()},
                {ExportFeedYandexDirectDeliveryCost.GlobalDeliveryCost.ToString(),ExportFeedYandexDirectDeliveryCost.GlobalDeliveryCost.Localize() },
                {ExportFeedYandexDirectDeliveryCost.LocalDeliveryCost.ToString(),ExportFeedYandexDirectDeliveryCost.LocalDeliveryCost.Localize() },
            };
    ...
    public class ExportFeedSettingsYandexDirectPromoModel : IValidatableObject
    {
        ...
        public IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
        {
            var type = Type.TryParseEnum<YandexDirectPromoType>();
            if (StartDate != null && ExpirationDate != null)
            {
                var delta = ExpirationDate - StartDate;
                if (type == YandexDirectPromoType.Flash && delta.Value.Days > 7)
                {
            ...
            if (type == YandexDirectPromoType.PromoCode)
            {
                if (CouponId <= 0)
            ...
            else
            {
                ...
                {
                    if(type == YandexDirectPromoType.NPlusM )
                    {
                ...
                if (type == YandexDirectPromoType.Gift)
                {
