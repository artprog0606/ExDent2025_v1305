    ...
    public class IPTelephonyHandler
    {
        ...
        private object GetNotificationData(Customer customer)
        {
            ...
            if (haveCrm)
            {
                ...
                {
                    var orderSource = IPTelephonyService.GetOrderSource(_call.DstNum) ??
                                      OrderSourceService.GetOrderSource(OrderType.Phone);

                    lead = new Lead
                    {
                        Phone = customer.StandardPhone.ToString(),
                        CustomerId = customer.Id,
                        Customer = customer.Id != Guid.Empty ? customer : null,
