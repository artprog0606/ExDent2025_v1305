    ...
    public class GetCardHandler
    {
        ...
        private void GetPaging()
        {
            ...
            _paging
                .Select(
                    ...
                    "CardNumber",
                    "Email",
                    "StandardPhone".AsSqlField("MobilePhone"),
                    "isnull(LastName,'') + ' ' + isnull(FirstName,'')".AsSqlField("FIO"),
                    "Grade.Name".AsSqlField("GradeName"),
                    "Grade.BonusPercent".AsSqlField("GradePersent"),
                    "CreateOn".AsSqlField("Created"))
                .From("[Bonus].[Card]")
                .Inner_Join("[Bonus].Grade ON Grade.Id=Card.GradeId")
                .Inner_Join("[Customers].Customer ON Customer.CustomerId=Card.CardId");
        ...
        private void Filter()
        {
            ...
            if (_filterModel.BonusAmountFrom.HasValue || 
                _filterModel.BonusAmountTo.HasValue)
            {
                if (_filterModel.BonusAmountFrom.HasValue)
                    _paging.Where(
                        "(Select ISNULL(Sum(Amount), 0) " +
                        "From Bonus.Bonuses " +
                        "Where Bonuses.CardId = Card.CardId " +
                                "and (EndDate is null or EndDate >= {0}) " +
                                "and (StartDate is null or StartDate <= {0}) " +
                                "and Amount > 0 " +
                                "and Status <> {1}) >= {2}",
                        DateTime.Today,
                        (int)EBonusStatus.Removed,
                        _filterModel.BonusAmountFrom.Value);
                ...
                if (_filterModel.BonusAmountTo.HasValue)
                    _paging.Where(
                        ...
                        DateTime.Today,
                        (int)EBonusStatus.Removed,
                        _filterModel.BonusAmountTo.Value);
            }
            ...
            {
                long? standardPhone = StringHelper.ConvertToStandardPhone(_filterModel.MobilePhone, true, true);
            ...
            if (_filterModel.CustomerGroupId.HasValue)
            {
                _paging.Where("[Customer].CustomerGroupId = {0}", _filterModel.CustomerGroupId);
            }
