...
namespace AdvantShop.Web.Admin.Handlers.Cms.Carousel
{
    public class GetCarousel
    {
        ...
        public FilterResult<CarouselResultFilterModel> Execute()
        {
            ...
            model.TotalString = string.Format(LocalizationService.GetResource("Admin.Carousel.Index.ImagesFound"), model.TotalItemsCount);
        ...
        private void Filter()
        {
            ...
            {
                _filterModel.CarouselUrl = _filterModel.Search;
            }

            if (!string.IsNullOrWhiteSpace(_filterModel.CarouselUrl))
            {
                _paging.Where("URL LIKE '%'+{0}+'%'", _filterModel.CarouselUrl);
            }
